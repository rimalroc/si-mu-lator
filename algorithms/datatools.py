...
def training_prep(X, sig_keys):
    ...
    X_out = np.zeros((X.shape[0], X.shape[1], X.shape[2]+2))
    ...
    for iev in tqdm(range( X.shape[0] )):
        ...
        X_out[iev,valid_hits,:sig_keys.index('ptypetilt')] = np.copy(ev[valid_hits,:])
        
        #is_signal
        X_out[iev,valid_hits,sig_keys.index('is_signal')] = np.ones_like(X_out[iev,valid_hits, -1])
            
        for sk in sig_keys:
            if 'is_signal' in sk or 'ptypetilt' in sk: 
                continue
            elif 'time' in sk:
        ...
        #ptypetilt
        X_out[iev,valid_hits, sig_keys.index('ptypetilt')] = np.copy( X_out[iev, valid_hits,sig_keys.index('ptilt')] ) + 3
        ...
        X_out[iev,valid_hits_stg,sig_keys.index('time')] = np.zeros_like(X_out[iev, valid_hits_stg,sig_keys.index('time')])
        
        X_out[iev,valid_hits_stg,sig_keys.index('ptypetilt')] = np.ones_like(X_out[iev, valid_hits_stg,sig_keys.index('ptilt')])
