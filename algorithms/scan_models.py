...
for mloc in allmodels:
    ...
    mdict['mod_mse_y'] = metrics.mean_squared_error( data['ev_mu_y'][this_cut], y_reg[this_cut] )
    ...
    mdict['mod_dy_median'] = np.median( data['ev_mu_y'][this_cut] - y_reg[this_cut] )
    ...
    mdict['mod_dy_iqr'] = iqr( data['ev_mu_y'][this_cut] - y_reg[this_cut], rng=(16, 84) )
    ...
    mdict['skew_dy'] = skew( data['ev_mu_y'][this_cut] - y_reg[this_cut] )
    ...
    mdict['dy_mmm'] = ( mdict['mod_dy_mean'] - mdict['mod_dy_median'] ) / mdict['mod_dy_std']
    ...
    dy_hist = np.histogram( ( data['ev_mu_y'][this_cut] - y_reg[this_cut] ), range=(-20,20), bins=100 )
    dy_bins = 0.5*( dy_hist[1][:-1] + dy_hist[1][1:] )
    ...
    mdict['dy_mode'] = dy_bins[ np.argmax( dy_hist[0] ) ]
