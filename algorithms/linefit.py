...
def get_mm_line_fit(hits, sig_keys, is_perfect=False, only_x=False):
    ...
    xs = hits[:, sig_keys.index('projX_at_middle_x')].copy()
...
# def get_mdt_line_fit(hits, sig_keys, is_perfect=False, only_x=False):

#     # print( hits.size/len( sig_keys ) )
#     ## extract quantities
#     hit_types = hits[:, sig_keys.index('is_muon')]
#     valid_hits = hit_types > -90
...
#     fit_hits = hits[indx_hit]

#     zs = fit_hits[:, sig_keys.index('z')]
#     xs = fit_hits[:, sig_keys.index('projX_at_middle_x')].copy()
#     xends = fit_hits[:, sig_keys.index('projX_at_rightend_x')]
