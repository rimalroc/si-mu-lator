...
parser = argparse.ArgumentParser(description='Run batch at CERN')
parser.add_argument('-m', '--addmuon', dest='ismu', action='store_true', default=False,
                    help='Simulate muon')
parser.add_argument('-b', '--disable-background', dest='bkg', action='store_false', default=True,
                    help='Simulate muon')
...
if 'slac' in domain:
    ...
    exec_file =  '''#!/bin/bash
APPTAINER_IMAGE_PATH=/sdf/sw/ml/slac-ml/20200227.0/slac-jupyterlab@20200227.0.sif
apptainer exec --nv -B /sdf,/gpfs,/scratch,/lscratch ${APPTAINER_IMAGE_PATH} python _ROOTDIR_/si_mu_late.py _OPTIONS_ -r $1 _det_noise_
'''
elif 'cern' in domain:
    exec_file =  '''#!/bin/bash
APPTAINER_IMAGE_PATH=/Data/images/slac-jupyterlab@20200227.0.sif
apptainer exec  -B /Data -B /afs ${APPTAINER_IMAGE_PATH} python _ROOTDIR_/si_mu_late.py _OPTIONS_ -r $1 _det_noise_
 '''
else:
        exec_file =  '''#!/bin/bash
APPTAINER_IMAGE_PATH=/your/sing/image
apptainer exec --nv  ${APPTAINER_IMAGE_PATH} python _ROOTDIR_/si_mu_late.py _OPTIONS_ -r $1 _det_noise_       
'''
...
print (f"run_batch_cern: going to run {exec_file_name} {njobs} times")
